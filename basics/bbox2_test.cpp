...
TEST_CASE("bbox2")
{
	...
	SECTION("constructor_1") {
		...
		for (int i = 0; i < 3; ++i) {
			bool t = bb.bounds[i][0] == std::numeric_limits<float>::infinity() &&
	...
	SECTION("constructor_2") {
		...
		REQUIRE(bb.bounds[0][2] == 8.0);
		REQUIRE(bb.bounds[0][3] == 9.0);

		REQUIRE(bb.bounds[1][0] == -2.5);
		REQUIRE(bb.bounds[1][1] == -3.5);
		REQUIRE(bb.bounds[1][2] == 7.25);
		REQUIRE(bb.bounds[1][3] == 8.25);

		REQUIRE(bb.bounds[2][0] == 0.5);
		REQUIRE(bb.bounds[2][1] == 1.5);
		REQUIRE(bb.bounds[2][2] == 2.0);
		REQUIRE(bb.bounds[2][3] == 3.0);
	}
