...
class MicroSurface
{
	 ...
	 * MicroSurface tree.
	 */
	bool intersect_node(size_t node, const Ray &ray, const Vec3 inv_d, const std::array<uint32_t, 3> d_is_neg, float *tnear, float *tfar, float *t) {
		uint32_t ti = 0;
		float alpha = 0.0f;
		if (calc_time_interp(time_count, ray.time, &ti, &alpha)) {
			const BBox b = lerp<BBox>(alpha, nodes[node+ti].bounds, nodes[node+ti+1].bounds);
			return b.intersect_ray(ray, inv_d, d_is_neg, tnear, tfar, t);
		} else {
			return nodes[node+ti].bounds.intersect_ray(ray, inv_d, d_is_neg, tnear, tfar, t);
		}
	}
